...
export const suggestCategory = (title: string, description: string) => {
  ...
  if (text.includes("code") || text.includes("program") || text.includes("dev")) return "Coding";
  if (text.includes("exam") || text.includes("test") || text.includes("quiz")) return "Exam";
  if (text.includes("read") || text.includes("book") || text.includes("essay")) return "Reading";
  return "General";
};
